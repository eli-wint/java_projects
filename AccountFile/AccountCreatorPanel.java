...
public class AccountCreatorPanel {
    ...
    public static void main(String[] args) {
        ...
        panel.add(createAccountButton, gBagConst);
        gBagConst.gridy = 1;
        panel.add(loginButton, gBagConst);
        ...
        loginButton.addActionListener((ActionEvent b) -> {
            if (b.getSource() == loginButton) {
                ...
                panel.add(label, gBagConst);
                gBagConst.gridx = 1;
                panel.add(textInput, gBagConst);
                gBagConst.gridx = 2;
                panel.add(enterButton,gBagConst);
                gBagConst.gridx = 1;
                gBagConst.gridy = 1;
                panel.add(suggestionLabel,gBagConst);
                frame.revalidate();
                frame.repaint();
            }
        });
                    ...
                    if (validUsernames.contains(textInput.getText().toLowerCase())) {
                        ...
                        if (isCheckingPassword) {
                            ...
                            if (FileIO.ReadFile("AccountUsernames.txt", false).contains(passedUsername.toLowerCase() + "  " + textInput.getText().toLowerCase())){
                                ...
                                gBagConst.gridx = 0;
                                gBagConst.gridy = 2;
                                gBagConst.weightx = 100;
                                gBagConst.fill = gBagConst.HORIZONTAL;
                                panel.add(menu, gBagConst);
                                gBagConst.gridy = 1;
                                panel.add(label, gBagConst);
                                menu.addItem("Home");
